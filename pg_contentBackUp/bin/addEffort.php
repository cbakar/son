...
<?php
	...
	$query = "INSERT INTO forum_effort ( `user_name`, `effort_number`, `fisher_name`, `bt_size`, `effort_date`, `entered_on`, `grid`, `latlong`, `grtp`, `grlen5`, `grhtm`, `mesh5`, `grdep5`, `sidep5`, `setype`, `effdur` ) 
	VALUES(	?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )";
	$stmt = $conn->prepare($query);

	//bind parameters for markers, where (s = string, i = integer, d = double,  b = blob)
	$stmt->bind_param('ssssssssssssssss', $entered_by, $effortNumber, $fisherName, $btSize, $effortDate, $entered_on, $grid, $latlong, $grtp, $grlen5, $grhtm, $mesh5, $grdep5, $sidep5, $setype, $effdur);

	if($stmt->execute()){
		$stmt->close();
		$conn->close();
		header( 'Location: ../usrPg.php?id=effdata_added');
		//print 'Success! ID of last inserted record is : ' .$stmt->id .'<br />';
	}else{
	...
	// if( !$insert_row ) { closeDB(); header( 'Location: ../usrPg.php?id=hdaw197ent_err'); }
